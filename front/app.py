...
st.set_page_config(
    page_title="Waterflow 2 - Portail Expert", page_icon="💧", layout="centered"
)
...
if os.path.exists("/.dockerenv"):
    default_api_url = "http://api-unique:8000/api"
else:
    default_api_url = "http://localhost:8000/api"

API_BASE_URL: str = os.getenv("API_URL", default_api_url)
...
with col1:
    ph = st.slider(
        "pH (Potentiel Hydrogène)", min_value=0.0, max_value=14.0, value=7.2, step=0.1
    )
    hardness = st.number_input("Dureté (mg/L)", min_value=0.0, value=200.0, step=1.0)
    solids = st.number_input(
        "Solides totaux (ppm)", min_value=0.0, value=20000.0, step=100.0
    )
    chloramines = st.slider(
        "Chloramines (ppm)", min_value=0.0, max_value=15.0, value=3.2, step=0.1
    )

with col2:
    sulfate = st.number_input("Sulfates (mg/L)", min_value=0.0, value=330.0, step=1.0)
    conductivity = st.number_input(
        "Conductivité électrique (μS/cm)", min_value=0.0, value=420.0, step=1.0
    )
    organic_carbon = st.slider(
        "Carbone Organique Total (ppm)",
        min_value=0.0,
        max_value=30.0,
        value=14.2,
        step=0.1,
    ...
    trihalomethanes = st.slider(
        "Trihalométhanes (μg/L)", min_value=0.0, max_value=130.0, value=66.3, step=0.1
    )

turbidity = st.slider(
    "Turbidité de l'eau (NTU)", min_value=0.0, max_value=7.0, value=3.8, step=0.1
)

if st.button("Lancer l'analyse de l'échantillon", type="primary"):
    if not user_api_key:
        st.warning(
            "⚠️ L'accès à l'inférence requiert une clé API valide dans la barre latérale."
        )
    else:
        ...
        with st.spinner("Transmission sécurisée et calcul par l'API..."):
            try:
                ...
                if response.status_code == 200:
                    ...
                    if prediction == 1:
                        st.success(
                            f"### 🎉 Diagnostic : L'eau est estimée **{status_label}** !"
                        )
                    else:
                        st.error(
                            f"### ❌ Diagnostic : L'eau est estimée **{status_label}** (Impropre)."
                        )
                        if "decision_reason" in result:
                            st.info(f"ℹ️ {result['decision_reason']}")
                elif response.status_code == 401:
                    st.error(
                        "🚫 Authentification refusée : Clé API invalide ou révoquée."
                    )
                else:
                    st.error(
                        f"❌ Erreur API (Code {response.status_code}) : {response.text}"
                    )
...
if fichier_importe is not None:
    if st.button("Lancer l'extraction OCR", type="secondary"):
        if not user_api_key:
            st.warning(
                "⚠️ Une clé API est obligatoire pour enregistrer les données extraites par OCR."
            )
        else:
            files_payload = {
                "file": (
                    fichier_importe.name,
                    fichier_importe.getvalue(),
                    fichier_importe.type,
                )
            }
            headers = {"X-API-Key": user_api_key}
            ...
            with st.spinner("Numérisation et extraction par l'API OCR.space..."):
                try:
                    ...
                    if response_ocr.status_code == 201:
                        ...
                        st.success(
                            f"🎉 Analyse terminée ! ID Prélèvement généré : {data_json['prelevement_id']}"
                        )
                        ...
                        st.error(
                            f"❌ Échec de l'extraction (Code {response_ocr.status_code}) : {response_ocr.text}"
                        )
                except Exception as e:
                    st.error(
                        f"🔌 Erreur réseau : Impossible de joindre le module OCR : {e}"
                    )
