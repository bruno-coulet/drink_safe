...
def ensure_models_are_loaded():
    ...
    if not ml_models:
        try:
            ...
            for model_name in [
                "WaterModel_LogisticRegression",
                "WaterModel_RandomForestClassifier",
                "WaterModel_XGBClassifier",
                "WaterModel_MLPClassifier",
            ]:
                try:
            ...
            # Loggue l'erreur globale de connexion à MLflow
            logger.warning(f"Erreur globale lors de la connexion à MLflow : {e}")


# Décorateur pour ignorer le test si le dataset de validation standardisé n'est pas présent sur github
@pytest.mark.skipif(
    not os.path.exists("data/processed/water_std.csv"),
    reason="Dataset ignoré sur Git (CI)",
)
def test_non_regression_performance_catalogue() -> None:
    ...
    assert os.path.exists(filepath_val), (
        f"Le fichier de validation standardisé est introuvable : {filepath_val}"
    )
    ...
    assert "Potability" in df_val.columns, (
        "La colonne cible 'Potability' est absente du fichier de validation"
    )
    ...
    if not ml_models:
        pytest.skip(
            "Aucun modèle n'est actuellement chargé dans ml_models (le Model Registry ou le serveur de tracking est injoignable)."
        )
    ...
    for model_name, model in ml_models.items():
        ...
        assert score_f1 >= 0.60, (
            f"Le modèle {model_name} a subi une régression sur le F1-score : {score_f1:.2f} < 0.60"
        )
        ...
        try:
            ...
            elif hasattr(model, "_model_impl") and hasattr(
            ...
            assert auc_pr >= 0.50, (
                f"Le modèle {model_name} a un AUC-PR insuffisant : {auc_pr:.2f} < 0.50"
            )
        except Exception as e: # noqa: BLE001
        ...
        assert total_predictions == len(y_val), (
            f"Incohérence dans la matrice de confusion pour {model_name} : {total_predictions} != {len(y_val)}"
        )
