...
class WaterSample(BaseModel):
    ...
    Trihalomethanes: float = Field(
        ..., description="Concentration en trihalométhanes en ppm"
    )
    Turbidity: float = Field(..., description="Turbidité de l'eau en NTU")
    observations: str | None = Field(None, description="Note contextuelle optionnelle")
...
def _garde_fou_oms(payload: WaterSample) -> str | None:
...
def _charger_modele(model_choice: str) -> tuple[Any, str]:
    ...
    """
    from src.api import ml_model_versions, ml_models

    # Si le modèle est déjà présent dans la mémoire vive (préchargé par src/api.py au démarrage)
    # l'inférence se lance instantanément. Aucun accès disque n'est fait.
    model = ml_models.get(model_choice)
    if model is not None:
        return model, ml_model_versions.get(model_choice, "inconnue")

    # sinon tente de le charger depuis MLflow Model Registry (lazy loading)
    print(
        f"[Lazy Load] Modèle '{model_choice}' absent du cache. Recherche de la dernière version..."
    )
    try:
        client = MlflowClient()
        ...
        # va physiquement lire le fichier binaire sur le volume mlruns_artifacts
        model = mlflow.sklearn.load_model(model_uri)
        # l'injecte dans le dictionnaire ml_models pour que les prochains utilisateurs
        # n'aient plus à subir ce temps de chargement.
...
def _inferer(model: Any, payload: WaterSample) -> tuple[int, float | None]:
...
def executer_prediction(
    ...
    try:
        ...
        _sauvegarder_prelevement_en_bdd(
            ...
            payload,
            potability_result,
            f"{payload.model_choice}_v{version_modele}",
        )
        return {
        ...
        raise HTTPException(
            ...
            detail=f"Erreur lors du calcul de la prédiction : {e}",
        )
...
def _charger_prelevement(prelevement_id: int, client_id: str) -> WaterSample:
    ...
    """
    try:
        with psycopg2.connect(settings.DATABASE_URL) as conn, conn.cursor(cursor_factory=RealDictCursor) as cursor:
...
def _maj_prediction_prelevement(
    prelevement_id: int, client_id: str, prediction: int, model_ver: str
) -> None:
    ...
    try:
        with psycopg2.connect(settings.DATABASE_URL) as conn, conn.cursor() as cursor:
            cursor.execute(
                query, (prediction, model_ver, prelevement_id, client_id)
            )
            conn.commit()
    except Exception as e:  # noqa: BLE001
        print(
            f"Alerte MLOps : Impossible de mettre à jour la prédiction du prélèvement : {e}"
        )


def _sauvegarder_prelevement_en_bdd(
    client_id: str, payload: WaterSample, prediction: int, model_ver: str
) -> None:
