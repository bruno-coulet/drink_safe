...
class MeasurementCreate(BaseModel):
    ...
    lieu: str | None = Field(None, description="Lieu / point de prélèvement")
    observations: str | None = Field(
        None, description="Remarques ou contexte du prélèvement"
    )


@router.post("/", status_code=status.HTTP_201_CREATED)
def deposer_mesures(
    payload: MeasurementCreate, client_id: str = Depends(get_current_client)
) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Échec de l'écriture en base de données : {e}",
        )


@router.get("/")
def lister_prelevements_client(
    ...
    """
    query_select: str = (
        "SELECT * FROM prelevements WHERE client_id = %s ORDER BY cree_le DESC;"
    )
    try:
...
def vue_globale_admin(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erreur d'accès à la table globale : {e}",
        )
