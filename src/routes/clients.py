...
from src.dependencies.auth import get_admin_user, hacher_cle
...
class ClientCreate(BaseModel):
    ...
    client_id: str = Field(
        ..., max_length=50, description="Identifiant unique du client (ex: LOR_EAU_01)"
    )
    denomination: str = Field(
        ..., max_length=100, description="Nom officiel ou raison sociale de l'entité"
    )
    adresse: str = Field(..., description="Adresse postale complète du client")
    role: str | None = "client"  # Par défaut, on crée un client normal
...
def creer_client(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Erreur d'insertion : {e}")
...
def get_mon_profil(api_key: str = Security(api_key_header)) -> Dict[str, Any]:
    ...
    try:
        ...
        if not client:
            raise HTTPException(
                status_code=401, detail="Clé API invalide ou introuvable."
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erreur interne de la base de données : {e}"
        )
