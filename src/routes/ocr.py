...
async def ingerer_fiche_laboratoire(
    file: UploadFile = File(
        ..., description="Fiche laboratoire au format PDF, PNG ou JPG"
    ),
    ...
    type_contenu = file.content_type or "application/octet-stream"
    ...
    fichiers_ocr = {"file": (nom_fichier, contenu_fichier, type_contenu)}
    ...
    try:
        with psycopg2.connect(settings.DATABASE_URL) as conn:
            with conn.cursor() as cursor:
                ...
                ligne_creation = cursor.fetchone()
                if ligne_creation is None:
                    raise RuntimeError(
                        "L'identifiant du prélèvement OCR n'a pas pu être récupéré."
                    )
                prelevement_id: int = ligne_creation[0]
                conn.commit()
