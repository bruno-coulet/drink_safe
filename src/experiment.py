...
def executer_pipeline_mlops() -> None:
    ...
    if not os.path.exists(path_brut) or not os.path.exists(path_std):
        raise FileNotFoundError(
            "Les fichiers de données dans 'data/processed/' sont introuvables."
        )
    ...
    for nom_modele, instance_modele in modeles.items():
        ...
        print(
            f"[MLOps] 🚀 Lancement du Run MLflow pour : {nom_modele} ({type_data})..."
        )
        ...
        with mlflow.start_run(run_name=f"Run_{nom_modele}"):
            ...
            print(
                f"✓ {nom_modele} correctement entraîné et poussé dans le registre sous : {nom_registre}"
            )
