...
class Settings:
    ...
    DB_USER: str = os.getenv("POSTGRES_USER", "admin_waterflow")
    DB_PASSWORD: str = os.getenv("POSTGRES_PASSWORD", "MonMotDePasseSecurise123!")
    DB_NAME: str = os.getenv("POSTGRES_DB", "waterflow_db")

    DATABASE_URL: str = (
        f"postgresql://{DB_USER.strip()}:{DB_PASSWORD.strip()}@"
        f"{DB_HOST.strip()}:5432/{DB_NAME.strip()}"
    )
